...
def test_loop(dataloader, model, loss_fn, t):
    ...
    with open(f'logs/epoch_{i}_CC_{t}_{name_data}.txt', 'w') as archivo:
...
for t in range(total_fase):
    ...
    if save_model: torch.save(model.state_dict(), f'Fase_{t}_{name_data}.pth'); print(f"Se guardo el modelo en la Fase:{t}_{name_data}")


with open(f'logs/log_accuracy_loss_{name_data}.txt', 'w') as archivo:
        # Escribe el valor de la variable en el archivo
