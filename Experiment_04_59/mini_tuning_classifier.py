...
subset = torch.utils.data.Subset(train_data, sampled_indices)

# Crear un DataLoader para el conjunto de datos
train_dataloader = torch.utils.data.DataLoader(subset, batch_size=32, shuffle=True)
...
def test_loop(dataloader, model, loss_fn):
    ...
    with open(f'logs/epoch_{i}_mini_Tuning.txt', 'w') as archivo:
...
for t in range(total_fase):
    ...
    if save_model: torch.save(model.state_dict(), f'mini_Tuning.pth')


with open(f'logs/log_accuracy_loss_mini_Tuning.txt', 'w') as archivo:
        # Escribe el valor de la variable en el archivo
